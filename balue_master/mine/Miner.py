    ...
    def mine(self, metadata: str = "0") -> None:
        if len(blockchain.pending_blocks) > 0:
            ...
            if blockchain.pending_blocks[0].index != 0:
                previous_block: dict = blockchain.adjusts_path.storage.load_block(blockchain.get_index() - 1)
                if blockchain.validations.validate_block(blockchain.pending_blocks[0].to_dict(), previous_block):
                    blockchain.add_mined_block_to_chain()
                    blockchain.pending_blocks = []
            else:
                if blockchain.validations.validate_block(blockchain.pending_blocks[0].to_dict()):
                    blockchain.add_mined_block_to_chain()
                    blockchain.pending_blocks = []
        else:
