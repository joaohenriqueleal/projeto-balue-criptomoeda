...
class Blockchain:
    ...
    def __init__(self) -> None:
        self.chain_version: str = CHAIN_VERSION
        self.validations: 'Validations' = Validations()
        self.adjusts_path = self.validations.pending_validations.block_validations.transaction_validations.adjusts

        self.pending_blocks: list['Block'] = []
        self.create_genesis()

    def create_genesis(self) -> None:
        if len(self.adjusts_path.storage.chain) == 0:
    ...
    def new_pending_block(self) -> None:
        if len(self.pending_blocks) == 0:
            block: 'Block' = Block(
                ...
                self.adjusts_path.adjust_difficulty(self.get_index()),
                self.adjusts_path.adjust_reward(self.get_index())
            )
            self.pending_blocks.append(block)

    def get_index(self) -> int:
        return len(self.adjusts_path.storage.chain)

    def get_previous_hash(self) -> str:
        if len(self.adjusts_path.storage.chain) == 0:
            return "0" * 32
        return self.adjusts_path.storage.load_block(self.get_index() - 1)["hash"]

    def add_mined_block_to_chain(self) -> None:
        if len(self.pending_blocks) > 0:
            p: 'Block' = self.pending_blocks[0]
            if p.hash != "0" * 32 and p.miner_address and p.signature:
                self.adjusts_path.storage.add_block(p.to_dict())
                self.pending_blocks = []
